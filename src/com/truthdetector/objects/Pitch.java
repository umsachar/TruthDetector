...
public final class Pitch {
	 ...
	 *            The Pitch in Hertz.
	 */
	public Pitch(final double hertzValue) {
		this.pitchInHertz = hertzValue;
	}
	
	public Pitch(final PitchUnit unit, final double value) {
		switch (unit) {
		case ABSOLUTE_CENTS:
			pitchInHertz = PitchUnit.absoluteCentToHertz(value);
			break;
		case RELATIVE_CENTS:
			throw new IllegalArgumentException("Cannot convert relative cent value to absolute "
					+ "frequency. Pitch object creation failed.");
		case MIDI_KEY:
			pitchInHertz = PitchUnit.midiKeyToHertz((int) value);
			break;
		case MIDI_CENT:
			pitchInHertz = PitchUnit.midiCentToHertz(value);
			break;
		case HERTZ:
			pitchInHertz = value;
			break;
